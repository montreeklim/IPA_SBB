...
#     e = np.ones(len(E))
#     if cond_number >= 1e+7: # Learning to Scale MIP Timo and Gregor
...
def up_extension_constraint(vertices_matrix):
    ...
    cond_number = np.linalg.cond(E)
    e = np.ones(len(E))
    if cond_number >= 1e+4: # Learning to Scale MIP, Timo and Gregor, treat cond_number < 1e+7 as normal for MIP
        a = np.linalg.pinv(E) @ e
    else:
        a = np.linalg.solve(E, e)
...
def solveprob(prob):
    ...
    # prob.controls.optimalitytol = 1e-4
