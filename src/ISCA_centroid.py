...
def compute_w_gamma_y(a, x, w_old, rows, cols, BigM):
    ...
    for j in range(cols):
        ...
        # [v2: Improvement 5] scipy.linalg.eigh finds the smallest eigenpair of the
        # symmetric B_j directly — more reliable than inverse power iteration on
        # near-singular matrices (nearly coplanar clusters).
...
def cbbranch(prob, data, branch):
    ...
    # [v3: Fix 2] Read the per-ball evolved face for ball_id.
...
def cbnewnode(prob, data, parentnode, newnode, branch):
    ...
    if parent_init_depth < K:
        # [v3: Fix 4] Each init-phase node records only the faces assigned so far.
    ...
    # [v3: Fix 2] face_per_ball tracks a separate (N x N) face for each ball.
    ...
    centroid = np.mean(orig_face, axis=0)
    pi_w = ProjectOnBall(centroid)
...
def solve(pdata: ProblemData) -> xp.problem:
    ...
    # [v3: Fix 4] submatrix/a_coeff are populated lazily by cbbranch on first call.
    ...
    # best-first throughout — no large breadthfirst needed because the init
    # phase now fans out lazily (N+1 branches per level) instead of creating
    # all (N+1)^K combos at once.  LP bounds guide the search from level 1.
...
if __name__ == "__main__":
    ...
    for filename in datasets_filenames:
        ...
        if results:
            ...
            outname = f"results_{filename[:-4]}_centroid.xlsx"
